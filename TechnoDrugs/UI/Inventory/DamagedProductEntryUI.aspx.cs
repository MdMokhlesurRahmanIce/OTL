...
{
    public partial class DamagedProductEntryUI : PageBase
    {
        #region properties
        string statusMessage = string.Empty;
        string mode = "";
        public DamagedProductEntryUI()
        {
        ...
        {
            var purchaseLedgerDetailsProviderList = new List<DamageProductDetailProvider> { new DamageProductDetailProvider() };           
        }
        ...
        {
            Response.Redirect("~/UI/Inventory/DamagedProductEntryUI.aspx");
            this.AlertNone(lblMsg);
        }        
        ...
        {            
            try
            {
                ...
                HttpContext.Current.Session[StaticInfo.Query] = "SELECT ReferenceNo AS [Reference No], CONVERT(VARCHAR(30),DamageEntryDate, 105) AS [Entry Date] FROM [dbo].DamagedProduct DP WHERE DP.DivisionID = " + divisionID + " ORDER BY DamageEntryDate DESC, ReferenceNo DESC";
